 ...
 */
public class MetadataFactory extends BaseSchema {
	...
	public MetadataFactory(String vdbName, Object vdbVersion, String schemaName, Map<String, Datatype> runtimeTypes, Properties modelProperties, String rawMetadata) {
		...
		setDataTypes(runtimeTypes);
		this.builtinDataTypes = new TreeMap<String, Datatype>(String.CASE_INSENSITIVE_ORDER);
		this.builtinDataTypes.putAll(runtimeTypes);
		this.builtinDataTypes = Collections.unmodifiableMap(this.builtinDataTypes);
	...
	private void correctDataTypes(List<? extends BaseColumn> cols) {
		...
		for (BaseColumn c : cols) {
			...
			if (dt == null && this.dataTypes != null) {
				dt = this.dataTypes.get(name);
			}
			if (dt != null) {
				c.setDatatype(dt);
